...
K_THREAD_DEFINE(console_thread, 1024, console_thread_func, NULL, NULL, NULL,
...
static void subevent_result_cb(struct bt_conn *conn,
  ...
  LOG_INF(" - Procedure abort reason: %u",
...
static void remote_capabilities_cb(struct bt_conn *conn, uint8_t status,
  ...
  if (status == BT_HCI_ERR_SUCCESS) {
    ...
    LOG_INF(" - Max_Consecutive_Procedures_Supported: %u",
            params->max_consecutive_procedures_supported);
    LOG_INF(" - Num_Antennas_Supported: %u", params->num_antennas_supported);
    LOG_INF(" - Max_Antenna_Paths_Supported: %u",
            params->max_antenna_paths_supported);
    LOG_INF(" - Roles_Supported: 0x%02x",
            (params->initiator_supported ? (1 << 0) : 0) |
                (params->reflector_supported ? (1 << 1) : 0));
    LOG_INF(" - Modes_Supported: 0x%02x",
            params->mode_3_supported ? (1 << 0) : 0);
    LOG_INF(
        " - RTT_Capability: 0x%02x",
    ...
    LOG_INF(" - NADM_Sounding_Capability: 0x%04x",
            params->phase_based_nadm_sounding_supported ? (1 << 0) : 0);
    LOG_INF(" - NADM_Random_Capability: 0x%04x",
            params->phase_based_nadm_random_supported ? (1 << 0) : 0);
    LOG_INF(" - CS_SYNC_PHYs_Supported: 0x%02x",
            (params->cs_sync_2m_phy_supported ? (1 << 1) : 0) |
                (params->cs_sync_2m_2bt_phy_supported ? (1 << 2) : 0));
    LOG_INF(" - Subfeatures_Supported: 0x%04x",
            (params->cs_without_fae_supported ? (1 << 1) : 0) |
                (params->chsel_alg_3c_supported ? (1 << 2) : 0) |
                (params->pbr_from_rtt_sounding_seq_supported ? (1 << 3) : 0));
...
static int scan_init(void) {
  ...
  struct bt_scan_init_param param = {
      ...
      .conn_param =
          BT_LE_CONN_PARAM(0x14, 0x14, 0, BT_GAP_MS_TO_CONN_TIMEOUT(4000)),
      .connect_if_match = 1};
